...
const SOLIDITY_ARTICLE_1 = [
    "<h2>What is Filecoin</h2>",
    "<hr id='subTitle'>",
    "<p>Filecoin is a peer-to-peer network that stores files, with built-in economic incentives to ensure files are stored reliably over time.</p>",
    "<p>In Filecoin, users pay to store their files on storage providers. Storage providers are computers responsible for storing files and proving they have stored the files correctly over time. Anyone who wants to store their files or get paid for storing other users’ files can join Filecoin. Available storage, and the price of that storage, is not controlled by any single company. Instead, Filecoin facilitates open markets for storing and retrieving files that anyone can participate in.</p>",
    "<p>Filecoin includes a blockchain and native cryptocurrency (FIL). Storage providers earn units of FIL for storing files. Filecoin’s blockchain records transactions to send and receive FIL, along with proofs from storage providers that they are storing their files correctly.</p>",
    "<img id='imgArticle' src='./imgs/fi1.png' alt=''>",
    "<h4>For users</h4>",
    "<p>Filecoin enables users to store their files at hypercompetitive prices and verify that their files are being stored correctly.</p>",
    "<p>Users can choose their preferred trade-off between cost, redundancy, and speed by selecting the storage provider whose storage offer is best suited for their needs. Applications that implement Filecoin can negotiate storage with any storage provider on the network. Unlike centralized storage systems, there is no need to implement a different API for each provider.</p>",
    "<p>At any time, users can verify that their files are being stored correctly by looking at proofs on Filecoin’s blockchain.</p>",
    "<p>Filecoin is a peer-to-peer network that stores files, with built-in economic incentives to ensure files are stored reliably over time.</p>",
    "<h4>For storage providers</h4>",
    "<p>Filecoin enables storage providers to sell their storage on an open market.</p>",
    "<p>Storage providers are people and organizations that run storage providers, earning Filecoin tokens for doing so. A storage provider could be any internet-connected computer with spare disk space or a dedicated system with lots of storage built specifically for Filecoin. Filecoin’s blockchain rewards storage providers for contributing useful storage to the internet, not for completing wasteful proof-of-work computations.</p>",
    "<p>Once a storage provider implements the Filecoin protocol, they gain access to the entire market of Filecoin users. Providers don’t need to design their own storage API or advertise their offering themselves as this is handled by Filecoin’s protocol and network. Filecoin enables a thriving, decentralized ecosystem of independent storage providers by removing barriers to entry.</p>",
    "<p>See How providing storage works.</p>",
    "<h4>New to web3</h4>",
    "<p>If you are new to Filecoin or to the web3 space, welcome! We think you will enjoy getting to learn about some of the core concepts that make IPFS, Filecoin, Powergate, and other technologies possible as well as how to use them in your projects and applications.</p>",
    "<p>Web3 is a movement within software development that is turning centralized applications into decentralized protocols. Instead of having a single point of control, applications built on web3 architecture can remove the middle-man, reducing the risk of censorship by big corporations and services vulnerability to denial-of-service attacks. Web3 is a trustless infrastructure that allows users to have ownership and reclaim control over their data.</p>",
    "<p>Web3 is essentially what allows Filecoin to exist by allowing files to be shared between peers.</p>",
    "<p>You can find out more about it in IPFS and Filecoin section.</p>",
];

const ETHEREUM_ARTICLE_1 = [
    "<h2>What is Filecoin</h2>",
    "<hr id='subTitle'>",
    "<p>Filecoin is a peer-to-peer network that stores files, with built-in economic incentives to ensure files are stored reliably over time.</p>",
    "<p>In Filecoin, users pay to store their files on storage providers. Storage providers are computers responsible for storing files and proving they have stored the files correctly over time. Anyone who wants to store their files or get paid for storing other users’ files can join Filecoin. Available storage, and the price of that storage, is not controlled by any single company. Instead, Filecoin facilitates open markets for storing and retrieving files that anyone can participate in.</p>",
...
const CHAINLINK_ARTICLE_1 = [
    "<h2>Deploy Your First Smart Contract</h2>",
    "<hr id='subTitle'>",
    "<p>You can write your first smart contract and run it in your browser without any knowledge about Ethereum or blockchains. This guide shows you how easy it is to develop smart contracts using the Solidity language, a MetaMask wallet and the Remix Development Environment. You can use all of these tools in your browser for free with no signup required.</p>",
    "<p>In general, you create and deploy your smart contracts operate using the following process:</p>",
    "<p>Write: Write a contract to define how the contract functions, what data it can store, what other contracts it interacts with, and what external APIs it might call.</p>",
    "<p>Compile: Pass your smart contract code through a compiler to translate the contract into byte code that the blockchain can understand. For example, Solidity code must be compiled before it can run in the Ethereum Virtual Machine.</p>",
    "<p>Deploy: Send the compiled smart contract to the blockchain. From that point forward, the contract cannot be altered. However, you can still interact with the contract in several ways.</p>",
    "<p>Run functions: When you run the functions that you defined for the contract, the network processes those functions and modifies the state of your contract. For some functions, the network charges a small fee to complete the work. Your contract can also have functions that transfer funds to other contracts or wallets.</p>",
    "<h4>Install and fund your MetaMask wallet</h4>",
    "<p>Deploying smart contracts on-chain requires a wallet and ETH. The ETH pays for the work required by the Ethereum network to add the contract to the blockchain and store the variables. The wallet holds the ETH that you need to pay for the transaction. Install MetaMask, configure it to use the Kovan test network, and fund your wallet with free testnet ETH.</p>"
];

const FILECOIN_ARTICLE_1 = [
    "<h2>What is Filecoin</h2>",
    "<hr id='subTitle'>",
    "<p>Filecoin is a peer-to-peer network that stores files, with built-in economic incentives to ensure files are stored reliably over time.</p>",
    "<p>In Filecoin, users pay to store their files on storage providers. Storage providers are computers responsible for storing files and proving they have stored the files correctly over time. Anyone who wants to store their files or get paid for storing other users’ files can join Filecoin. Available storage, and the price of that storage, is not controlled by any single company. Instead, Filecoin facilitates open markets for storing and retrieving files that anyone can participate in.</p>",
...
const SOLIDITY_ARTICLES = [
    SOLIDITY_ARTICLE_1
];


const ETHEREUM_ARTICLES = [
    ETHEREUM_ARTICLE_1
];


const TRUFFLE_ARTICLES = [
    TRUFFLE_ARTICLE_1
];


const OPENZEPPELIN_ARTICLES = [
    OPENZEPPELIN_ARTICLE_1

];

const CHAINLINK_ARTICLES = [
    CHAINLINK_ARTICLE_1
];

const FILECOIN_ARTICLES = [
    FILECOIN_ARTICLE_1
];
